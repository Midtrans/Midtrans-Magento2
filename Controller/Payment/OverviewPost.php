...
class OverviewPost extends \Magento\Multishipping\Controller\Checkout
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            // Midtrans Handle Multishipping orders
            $paymentCode = $paymentInstance->getMethod();
            if ($paymentCode === 'snap' || $paymentCode === 'specific' || $paymentCode === 'installment') {
                $magentoBaseUrl    = $this->_objectManager->get('\Magento\Store\Model\StoreManagerInterface')->getStore()->getBaseUrl();
                $midtransUrl = $magentoBaseUrl . 'snap/payment/multishipping/';

                $this->_getState()->setActiveStep(State::STEP_SUCCESS);
                $this->_getCheckout()->getCheckoutSession()->clearQuote();
                $this->_getCheckout()->getCheckoutSession()->setDisplaySuccess(true);
                $this->_redirect($midtransUrl);
            } else {
                $this->_getState()->setActiveStep(State::STEP_SUCCESS);
                $this->_getCheckout()->getCheckoutSession()->clearQuote();
                $this->_getCheckout()->getCheckoutSession()->setDisplaySuccess(true);
